"""AgentFx function registry for AI agent integration.

Provides automatic discovery and JSON Schema generation for all AgentFx
...
Example:
    >>> from agentfx.registry import get_tool_schemas, invoke_tool
...
def _discover_modules() -> list[tuple[str, Any]]:
    """Discover all AgentFx submodules containing public functions.
    ...
    for pkg_info in pkgutil.iter_modules(agentfx.__path__):

        if not pkg_info.ispkg or pkg_info.name in _SKIP_MODULES:
            continue

        subpkg_name = f"agentfx.{pkg_info.name}"
...
def invoke_tool(tool_name: str, arguments: dict[str, Any]) -> Any:
    """Invoke a AgentFx tool by name with the given arguments.
...
def _get_semantic_engine() -> Any:
    ...
    try:
        from agentfx.semantic_search import SemanticToolSearch
