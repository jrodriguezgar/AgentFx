...
> Expose a big library of AgentFx functions as tools for AI agents via the Model Context Protocol.

## Overview

The AgentFx MCP server allows AI agents to **discover, search, and execute** any AgentFx function through a standardized protocol. Instead of writing Python code, an agent can call `call_agentfx("fxExcel.math_formulas.ABS", '{"number": -5.5}')` and get the result directly.

### What is MCP?

[Model Context Protocol (MCP)](https://modelcontextprotocol.io/) is an open standard that enables AI models to interact with external tools and data sources. AgentFx implements an MCP server that exposes 4 meta-tools which give agents access to the entire library.

### Why Dynamic Tool Definition?

AgentFx contains **2700+ functions** — registering each one as an individual MCP tool would:
...
Instead, AgentFx uses a **dynamic discovery pattern**: the MCP server registers only a handful of meta-tools (`search_agentfx_tools`, `list_agentfx_tools`, `get_agentfx_tool_details`, `call_agentfx`, `scientific_calculate`, etc.), and the AI agent uses them to discover, inspect, and invoke any of the 2700+ underlying functions at runtime.
...
| `__main__.py` | Entry point for `python -m agentfx.mcp` |
...
Once configured, an agent can interact with AgentFx using natural language. For example:
...
>
> ```
> call_agentfx("fxNumeric.finance_functions.future_value",
...
The server exposes 4 tools that together provide full access to AgentFx:
...
list_agentfx_tools("fxDate")

# List only Excel math functions
list_agentfx_tools("fxExcel.math_formulas")
...
by intent — not just exact keywords. Requires `fastembed` (`pip install agentfx[mcp-semantic]`).
...
search_agentfx_tools("round a number to 2 decimals")     # → ROUND, round_to_n_decimals
search_agentfx_tools("find a value in a table")          # → VLOOKUP, HLOOKUP, XLOOKUP
search_agentfx_tools("how many days between two dates")  # → calculate_days_between_dates, DAYS
search_agentfx_tools("calculate compound interest")      # → future_value, present_value, nper

# Also works with simple keywords
search_agentfx_tools("round")
search_agentfx_tools("vlookup")
```
...
```python
get_agentfx_tool_details("fxExcel.lookup_formulas.VLOOKUP")
```
...
Execute any AgentFx function by name.
...
```python
# Math
call_agentfx("fxExcel.math_formulas.ABS", '{"number": -5.5}')
# → {"result": 5.5}

# String manipulation
call_agentfx("fxString.string_operations.reverse_string", '{"input_string": "hello"}')
# → {"result": "olleh"}
...
# Date validation
call_agentfx("fxDate.date_operations.is_valid_date", '{"date_input": "2025-02-30"}')
# → {"result": false}

# VBA compatibility
call_agentfx("fxVBA.string_functions.Left", '{"string": "Hello World", "length": 5}')
# → {"result": "Hello"}
```
...
async def main():
    server = StdioServerParameters(command="python", args=["-m", "agentfx.mcp"])
...
An AI agent typically follows this pattern when using AgentFx MCP:

```
1. User asks: "What is the standard deviation of [4, 8, 15, 16, 23, 42]?"

2. Agent calls: search_agentfx_tools("standard deviation")
   → Finds: fxNumeric.statistics_functions.calculate_standard_deviation

3. Agent calls: get_agentfx_tool_details("fxNumeric.statistics_functions.calculate_standard_deviation")
   → Gets parameter schema: data (array), sample (boolean, default true)

4. Agent calls: call_agentfx("fxNumeric.statistics_functions.calculate_standard_deviation",
...
When installed with `pip install agentfx[mcp-semantic]`, the `search_agentfx_tools`
...
| Issue                                          | Solution                                                                                                  |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `ModuleNotFoundError: No module named 'mcp'` | Run `pip install agentfx[mcp]`                                                                        |
| Semantic search not activating                 | Run `pip install agentfx[mcp-semantic]` to install `fastembed`                                        |
...
| `Tool 'xxx' not found`                       | Use `search_agentfx_tools` or `list_agentfx_tools` to find the correct tool name                  |
| Connection refused / timeout                   | Verify the server starts correctly with `python -m agentfx.mcp`                                       |
