"""FormuLite MCP (Model Context Protocol) Server.

Exposes all FormuLite functions as MCP tools, enabling AI agents (Claude,
...
from formulite.registry import get_tool_schemas, invoke_tool, search_tools, warm_up_semantic_search

mcp = FastMCP(
    "FormuLite",
    instructions=(
        "FormuLite provides 1600+ reusable functions organized in 6 modules: "
        ...
        "\n\n"
        "IMPORTANT — Dynamic tool definition: FormuLite does NOT register each "
        "function as an individual MCP tool. Instead, use these meta-tools to "
        "discover and execute functions at runtime:\n"
        "1. 'search_formulite_tools' — find functions by natural language query "
        "(semantic search when fastembed is installed).\n"
        "2. 'list_formulite_tools' — browse available functions by module.\n"
        "3. 'get_formulite_tool_details' — get full parameter schema before calling.\n"
        "4. 'call_formulite' — execute any function by its qualified name "
...
@mcp.tool()
def list_formulite_tools(module: str = "") -> str:
    """List available FormuLite functions, optionally filtered by module.
    ...
    Example:
        list_formulite_tools("fxExcel.math")
    """
...
@mcp.tool()
def search_formulite_tools(query: str) -> str:
    """Search FormuLite functions using natural language or keywords.
    ...
    Example:
        search_formulite_tools("convert temperature")
    """
...
@mcp.tool()
def get_formulite_tool_details(tool_name: str) -> str:
    """Get full details of a specific FormuLite function including parameters.
    ...
    """
    from formulite.registry import get_tool_schema
...
@mcp.tool()
def call_formulite(tool_name: str, arguments: str = "{}") -> str:
    """Execute a FormuLite function by name with the given arguments.
...
def main() -> None:
    """Run the FormuLite MCP server over stdio."""
...
def get_scientific_constant(name: str) -> str:
    ...
    """
    from formulite.fxNumeric.constants_functions import _CONSTANTS
...
def list_calculator_functions() -> str:
    ...
    """
    from formulite.fxNumeric.calculator_functions import (
